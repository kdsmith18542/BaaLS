"""Submit second tx on local to make it height 2 (longer chain wins fork resolution)."""
...
# Build tx with nonce=2
tx_ts = int(time.time())
buf = bytes.fromhex(ALICE_PK)
buf += struct.pack("<Q", 2)  # nonce 2
...
buf += struct.pack("<I", 0) + struct.pack("<Q", 300)

tx_hash = hashlib.sha256(buf).digest()
sender_sk = SigningKey(bytes.fromhex(ALICE_SK))
tx_sig = sender_sk.sign(tx_hash).signature

tx = {"hash": list(tx_hash), "sender": list(bytes.fromhex(ALICE_PK)), "nonce": 2, "timestamp": tx_ts, "recipient": {"Wallet": list(bytes.fromhex(BOB_PK))}, "payload": {"Transfer": {"amount": 300}}, "signature": list(tx_sig), "gas_limit": 100000, "gas_price": 1, "priority": 0, "metadata": None, "chain_id": 1}
...
try:
    ...
    print(f"TX2 submitted: {urllib.request.urlopen(req).read().decode()}")
