...
pub trait Storage: Send + Sync {
    ...
    fn index_transaction(&self, tx_hash: &[u8; 32], block_hash: &[u8; 32], tx_index_in_block: u32) -> Result<(), StorageError>;
...
impl Storage for SledStorage {
    ...
    fn index_transaction(&self, tx_hash: &[u8; 32], block_hash: &[u8; 32], tx_index_in_block: u32) -> Result<(), StorageError> {
        let key = format!("block_tx:{}:{}:{:0>10}", hex::encode(block_hash), hex::encode(tx_hash), tx_index_in_block);
    ...
    fn get_transactions_by_block(&self, block_hash: &[u8; 32]) -> Result<Vec<Transaction>, StorageError> {
        ...
        for item in self.tx_by_block_tree.scan_prefix(prefix_string.as_bytes()) {
            ...
            let tx_hash_array: [u8; 32] = tx_hash_bytes.as_ref().try_into().map_err(|_| CryptoError::HashConversionError)?;
